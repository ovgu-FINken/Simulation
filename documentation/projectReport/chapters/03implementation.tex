...
\tikzset{
    group/.style={
           ...
           minimum width = 3cm,
            ...
            implements/.style={
           dashed straight line, closed, unfilled arrow pointing to interface class
           },
}
\begin{figure}[h]
\centering
...
\node[swScript, right=of vrep.45](finkenMeta){finkenMeta};
\node[swScript, below=of finkenMeta](finken){finken};
...
\node[swScript, above=of finkenCore](finkenPID){finkenPID};

\draw[-{open triangle 60}] (finken) to (finkenCore); 
\draw[-{angle 60}](finkenCore) to (finkenPID);
\draw[-{open triangle 60}, dashed](fink1) to (finken);
\draw[-{open triangle 60}, dashed](fink2) to (finken);
\end{tikzpicture}
...
\todo{FINKEN module as an API for further extensions}
