...
function getErrors(orientation, sensors, context, config)
	...
	if bottomDistance then
		...
		local wallDetectedFront, yawChangeRequiredFront = areDistancesToWall(frontDistance, leftDistance, rightDistance, context.counters.wallDetectedFront, config.wall_detection, config.suffix)
		local wallDetectedBack, yawChangeRequiredBack = areDistancesToWall(backDistance, leftDistance, rightDistance, context.counters.wallDetectedBack, config.wall_detection, config.suffix)
		local wallDetectedLeft, yawChangeRequiredLeft = areDistancesToWall(leftDistance, frontDistance, backDistance, context.counters.wallDetectedLeft, config.wall_detection, config.suffix)
		local wallDetectedRight, yawChangeRequiredRight = areDistancesToWall(rightDistance, frontDistance, backDistance, context.counters.wallDetectedRight, config.wall_detection, config.suffix)
		...
		local nextPosition = getAttractionRepulsionPosition(otherObjectPositions, context.randomPosition, config.attraction_repulsion, config.suffix)
...
function areDistancesToWall(middleDistance, leftDistance, rightDistance, counter, config, suffix)
...
-- Compares virtual ranging measurements with sonar to identify walls
function isFinken (vRangeList, dist)
	local eps = 0.286319104
	for i in string.gmatch(vRangeList,"%S+") do
		if (type(i)=="number" and i~=2.5 and math.abs(dist-i)<eps) then
			return true
		end
	end
	return false
...
function getAttractionRepulsionPosition(otherObjectPositions, targetPosition, config, suffix)
	local positionChange = {0, 0, 0}
	
	for _, otherObjectPosition in ipairs(otherObjectPositions) do
		local virtualRangingScriptHandle = simGetScriptAssociatedWithObject(getObjectHandle("Sensor_virtual_ranging",suffix))
		local vRangeList = simGetScriptSimulationParameter(virtualRangingScriptHandle, "dist")
		local euclideanNorm = getEuclideanNorm(otherObjectPosition)
		if (isFinken(vRangeList, euclideanNorm)) then
...
-- List of all finkens of the scene
SSE_AllFinkens = {}
