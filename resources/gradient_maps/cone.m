%creates a gradient map with all gradients pointing to a single point
% R: not used
...
%position of center/target point
target = [512 512];

r = zeros(size);
[coords1, coords2] = meshgrid(1:size, 1:size);
x_grad = target(1) - coords1;
y_grad = target(2) - coords2;
