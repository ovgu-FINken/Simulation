...
function newFinken(object, otherObjects)
	...
	local zPIDController = newPIDController(0.8, 0, 0)
	local throttlePIDController = newPIDController(10, 0, 0)
	...
	function move(targetPosition)
		...
		if (currentState == State.DEFAULT or currentState == State.ESCAPE) then
			...
			simAddStatusbarMessage("CP"..currentPosition[1].." "..currentPosition[2]..currentPosition[3])
			...
			simAddStatusbarMessage("CR"..currentRelative[1].." "..currentRelative[2]..currentRelative[3])
			...
			simAddStatusbarMessage("TY"..targetY)
			simAddStatusbarMessage("             CY"..currentY)
			...
			simAddStatusbarMessage("ROLL"..roll)
