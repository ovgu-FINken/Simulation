...
function newFinken(object, otherObjects)
	...
	function followLight(currentLightness, height)
		...
		local difference = math.abs(currentLightness - previousLightness)
		if(index % 5 == 0) then
			rnd = math.random()-0.5
		end
		local rndYaw = 0
		...
		if(previousLightness > currentLightness) then
			rndYaw = rnd * 2
			yawDirection = yawDirection + rndYaw
			simAddStatusbarMessage("GOOD DELTA:"..yawDirection)
		else
			rndYaw = rnd * 120
			yawDirection = yawDirection + rndYaw
			simAddStatusbarMessage(" BAD DELTA:"..yawDirection)
		end
		
		simAddStatusbarMessage("R == " .. difference)
		previousLightness = currentLightness
		
		--simAddStatusbarMessage("YAW:" .. yawDirection)
		--simAddStatusbarMessage("DIST" .. currentLightness)
		local target = {math.cos(math.rad(yawDirection)) * (difference+1),
						math.sin(math.rad(yawDirection)) * (difference+1),
						height}
		simSetObjectPosition(RelTarget, simGetObjectHandle("SimFinken"), {0, target[2], target[3]})
