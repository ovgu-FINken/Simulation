...
function newFinken(object, otherObjects)
	...
	function move(targetPosition)
		...
		--	simAddStatusbarMessage('pitch: ' .. tostring(pitchError))
		...
		--	simAddStatusbarMessage('roll: ' .. tostring(rollError))
			...
			if (throttleError > 5) then
				throttleError = 5
			elseif (throttleError < -5) then
				throttleError = -5
			end
			
			simSetScriptSimulationParameter(script, 'throttle', 50 + throttleError)
		--	simAddStatusbarMessage('throttle: ' .. tostring(throttleError))
