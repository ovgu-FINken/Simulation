...
--call the PID lua class here to create newPIDControllers
	local finkenPID		= require("finkenPID");

	local xPIDController = finkenPID.new()
	local yPIDController = finkenPID.new()
	local velocityPIDController = finkenPID.new()
	...
	--initialize all the PID's variables here
			xPIDController = finkenPID.init(6, 0, 8)
			yPIDController = finkenPID.init(6, 0, 8)
			velocityPIDController = finkenPID.init(1, 0, 1)
...
--pass the pitch0, pitch1 and pitch2 parameters in this simSetScriptSimulation Method
