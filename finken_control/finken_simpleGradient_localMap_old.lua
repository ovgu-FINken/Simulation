...
function finken.init(self)
    ...
    function self.UpdateLocalMapWithColorSensorValue()
           ...
           if(xSpeed > 0) then
                 ...
                 if(currentIndex > (yRes)) then
                    --shift array index to +1 and save prev. value to the new Index
                    prevValue = {}
                    prevValue[1] = localMapDataTable[currentIndex][1]
                    prevValue[2] = localMapDataTable[currentIndex][2]
                    
                    localMapDataTable[currentIndex]  = colorValue

                        if(currentIndexCol > 0) then
                            currentIndexCol = currentIndexCol - 1
                            currentIndex =((xRes/2) * yRes + currentIndexCol)
                            localMapDataTable[currentIndex] = prevValue
                        end
                 end
            end
            ...
            if(xSpeed < 0) then
                ...
                if(currentIndex < ((xRes*yRes) + yRes)) then
